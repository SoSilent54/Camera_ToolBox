...
```text
frontends
└── camera-toolbox  (唯一 bin；无参数启动 egui/eframe，有子命令走内部 CLI library)
   │
   ▼
...
手动操作是一等公民的可审计原子命令；半自动 workflow 是这些原子命令的受控编排，而不是另一套隐藏入口。GUI 与无头命令分支都支持人工调试、单步验证和脚本批处理，同时不把采集、寄存器访问、曝光状态机散落到前端。argv 分流发生在 eframe 初始化前，因此无头命令不连接图形显示服务。
