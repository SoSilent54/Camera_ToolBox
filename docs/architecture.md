...
Workflow::load_raw_and_analyze
   ...
   │    └ GUI 保留不可变 report.frame；core 执行 BLC/gain/bilinear/sRGB 纯计算，后台 worker latest-wins 重建彩色纹理
   ▼
...
| `ColorPipelineParams` / `PreparedBayer` | `core` | 校验四 CFA 通道 black/gain，并生成可双线性去马赛克的线性 Bayer 派生数据；不修改 `RawFrame`。 |
...
- 彩色纹理、rendered params、revision 与 diagnostics 必须同批安装；hover RGB 只按已安装参数计算，pending 参数不得提前影响显示读数。
