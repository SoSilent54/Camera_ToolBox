...
| JSON/YAML/EEPROM | 已实现 | JSON 保留完整 D12；YAML 仅保留 D5；EEPROM 写入 D8，并将 4 个 thin-prism 槽位强制清零，见第 12 节 |
...
| YAML | [`calibration_yaml.rs`](../crates/core/src/calibration_yaml.rs) | 固定 5 畸变参数兼容 YAML |
...
| `camera_intrinsics.yaml` | $f_x,f_y,c_x,c_y,k_1,k_2,p_1,p_2,k_3$, width, height | 既有 OpenCV/MATLAB pinhole-radtan 消费端 | **不保存** $k_4..k_6,s_1..s_4$，不等价于当前完整模型 |
...
```text
D_JSON   = [k1, k2, p1, p2, k3, k4, k5, k6, s1, s2, s3, s4]
D_YAML   = [k1, k2, p1, p2, k3]
D_EEPROM = [k1, k2, p1, p2, k3, k4, k5, k6,  0,  0,  0,  0]
```

只有 JSON 保留当前求解得到的完整 D12。EEPROM 虽然预留 12 个系数槽位，但 `s1..s4` 永远写 0，不能称为完整 D12；直接清零 thin-prism 也不等价于在 D8 模型下重新优化。

YAML 和 EEPROM 都必须使用独立数据验证降阶误差。对 EEPROM，应在完整标定 ROI 上比较相同 $K$ 下 D12 与 D8 去畸变映射：
...
| RMS 很低但四角校正异常 | 四角是否有观测、高阶系数是否不稳定、YAML 是否截断了模型 | 只看全局 RMS 接受结果 |
...
| JSON 与 YAML 去畸变不同 | YAML 只保存前 5 个畸变参数 | 把两者视为同一模型 |
