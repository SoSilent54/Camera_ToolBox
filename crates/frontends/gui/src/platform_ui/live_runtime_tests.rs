...
#[cfg(feature = "platform-ssh")]
#[test]
fn eeprom_targets_install_and_remove_exact_user_confirmed_cell() {
    use camera_toolbox_app::{EvidenceMaturity, SshEepromConfig, SshEepromTargetConfig};

    let mut profile = ssh_profile("ssh-a", "*.raw");
    let mode = SensorModeKey {
        sensor_id: SensorId::new("sensor-a").unwrap(),
        mode_id: "mode-a".to_owned(),
    };
    let PlatformConfig::SshManaged(config) = &mut profile.config else {
    ...
    let mut store = camera_toolbox_app::ProfileStore::new();
    store.insert_sensor(sensor()).unwrap();
    store.insert_platform(profile.clone()).unwrap();

    super::sync_eeprom_capability_declarations(&mut store, &profile, &Default::default()).unwrap();
    let cell = store.matrix().get(&mode, &profile.id).unwrap();
    let declaration = cell
        .declarations
        .iter()
        .find(|item| item.capability == CapabilityKind::EepromProvision)
        .unwrap();
    assert!(declaration.supported);
    assert_eq!(declaration.evidence, EvidenceMaturity::UserConfirmed);

    let old_targets = super::eeprom_target_keys(&profile);
    let PlatformConfig::SshManaged(config) = &mut profile.config else {
        panic!("expected SSH profile")
    };
    config.eeprom = None;
    super::sync_eeprom_capability_declarations(&mut store, &profile, &old_targets).unwrap();
    assert!(store.matrix().get(&mode, &profile.id).is_none());
}
