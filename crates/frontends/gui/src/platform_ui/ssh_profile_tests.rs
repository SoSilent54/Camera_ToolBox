...
#[test]
fn eeprom_editor_builds_only_typed_fixed_map_targets() {
    let mut state = SshEditorState::for_test();
    state.eeprom_enabled = true;
    state.eeprom_helper_program = "/opt/camera/bin/eeprom-helper".to_owned();
    state.eeprom_output_limit_bytes = 8192;
    state.eeprom_targets = vec![EepromTargetDraft {
    ...
    state.sync_eeprom_config(&mut config).unwrap();

    let eeprom = config.eeprom.unwrap();
    assert_eq!(eeprom.helper_program, "/opt/camera/bin/eeprom-helper");
    assert_eq!(eeprom.output_limit_bytes, 8192);
    assert_eq!(eeprom.targets.len(), 1);
    assert_eq!(eeprom.targets[0].sensor_mode.sensor_id.as_str(), "sensor-a");
    assert_eq!(eeprom.targets[0].sensor_mode.mode_id, "mode-a");
    assert_eq!(
        eeprom.targets[0].map_id,
        camera_toolbox_core::YG_STEREO_P24C64G_V1_MAP_ID
    );
    assert_eq!(eeprom.targets[0].i2c_bus, 7);
}

#[test]
fn eeprom_editor_rejects_incomplete_target_before_profile_commit() {
    let mut state = SshEditorState::for_test();
    state.eeprom_enabled = true;
    state.eeprom_targets = vec![EepromTargetDraft {
        sensor_id: "sensor-a".to_owned(),
    ...
    assert!(state.sync_eeprom_config(&mut config).is_err());
    assert!(config.eeprom.is_none());
}
