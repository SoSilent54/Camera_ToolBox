...
impl CalibrationEepromState {
    ...
    fn render_target_editor(&mut self, ui: &mut egui::Ui, sftp_source: Result<&str, &str>) {
        ui.collapsing("EEPROM SSH Target", |ui| {
            ...
            ui.weak(
                "Before each EEPROM operation, Camera Toolbox uploads the bundled companion helper to /usr/local/libexec/camera-i2c-helper, runs chmod 755, then reuses the selected SSH/SFTP control password and pinned SSH host key.",
            );
            if ui
                .add_enabled(
                    !self.busy && sftp_source.is_ok(),
    ...
    fn confirmed_provision_intent(
        ...
        let before_backup = self.device_backup.as_ref()?.clone();
...
mod tests {
    ...
    fn final_write_intent_requires_current_read_idle_state() {
        ...
        state.device_backup = Some(vec![0x11; 16]);
    ...
    fn failed_reconfiguration_invalidates_existing_read_authorization() {
        ...
        state.device_backup = Some(vec![0x22; 16]);
        assert!(
            state
