...
#[test]
fn x5_233_driver_sidebar_matches_workspace_adaptive_layout() {
    ...
    assert_eq!(x5.default_width, generic.default_width);
    assert_eq!(x5.min_width, generic.min_width);
    assert_eq!(x5.max_width_fraction, generic.max_width_fraction);
    assert_eq!(x5.max_width_cap, generic.max_width_cap);
    ...
    assert!(x5.max_width(1568.0) <= super::WORKSPACE_EXPLORER_MAX_WIDTH_CAP);
...
fn x5_233_yuv_snapshot_publishes_image_document() {
    ...
    let status = app
        .publish_x5_233_yuv_snapshot(
            &context,
            camera_toolbox_adapters::x5_tcp_client::X5YuvSnapshot {
                channel: 0,
                width: 2,
                height: 2,
                y_len: 4,
...
fn x5_233_raw_snapshot_publishes_raw_document() {
    ...
    let status = app
        .publish_x5_233_raw_snapshot(
            &context,
            camera_toolbox_adapters::x5_tcp_client::X5RawSnapshot {
                camera: 0,
                width: 2,
                height: 2,
                stride: 4,
...
#[cfg(feature = "calibration-opencv")]
#[test]
fn pangbot_simplified_flavor_renders_chinese_step_flow() {
    let context = egui::Context::default();
    context.enable_accesskit();
    let mut app =
        CameraToolboxApp::new_with_flavor(&context, super::UiFlavor::PangbotSimplified).unwrap();
    let mut frame = eframe::Frame::_new_kittest();
