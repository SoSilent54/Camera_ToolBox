...
use app::{CameraToolboxApp, UiFlavor};
...
fn run_gui() -> Result<()> {
    let flavor = UiFlavor::PangbotSimplified;
    tracing::debug!(operation = "gui_start", flavor = ?flavor, "starting GUI frontend");
    let native_options = eframe::NativeOptions::default();
    let window_title = "pangbot-calib-tool";
    eframe::run_native(
        window_title,
        native_options,
        Box::new(move |creation_context| {
            install_cjk_fonts(&creation_context.egui_ctx);
            CameraToolboxApp::new_with_flavor(&creation_context.egui_ctx, flavor)
    ...
    tracing::debug!(operation = "gui_exit", flavor = ?flavor, "GUI frontend exited");
    Ok(())
}
