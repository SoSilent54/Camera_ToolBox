...
use camera_toolbox_core::{Datum, I2cMapDefinition, builtin_i2c_map};
...
impl NodeInstance for SshConnectionNode {
    ...
    fn on_input(&mut self, _: &str, _: DataPacket, _: &mut NodeRuntime) -> Result<(), NodeError> {
        Ok(())
    }
...
impl NodeFactory for I2cFieldEncoderFactory {
    ...
    fn instantiate(&self, spec: NodeSpec) -> Result<Box<dyn NodeInstance>, NodeError> {
        Ok(Box::new(I2cFieldEncoderNode::new(spec)?))
    }
}
...
impl I2cFieldEncoderNode {
    ...
    fn emit_task_if_ready(&self, rt: &mut NodeRuntime) -> Result<(), NodeError> {
        if self
            .inputs
            .iter()
            .any(|input| input.required && !self.received.contains_key(&input.port_id))
        {
            return Ok(());
        }
        let mut fields = Vec::with_capacity(self.inputs.len());
        for input in &self.inputs {
            if let Some(field) = self.received.get(&input.port_id) {
                fields.push(field.as_ref().clone());
            }
        }
        let image = self.map.encode(&fields).map_err(|error| {
            NodeError::Precondition(format!("I2C encoder rejected fields: {error}"))
        })?;
        ...
        task.validate().map_err(|error| {
            NodeError::Precondition(format!("compiled I2C task is invalid: {error}"))
        })?;
        rt.emit(
            TASK_PORT,
            DataPacket::PacketData(Arc::new(
        ...
        rt.report_state(
            ...
            "I2C task compiled from encoded fields",
        );
        Ok(())
    }
}
...
impl NodeInstance for I2cFieldEncoderNode {
    ...
    fn on_start(&mut self, rt: &mut NodeRuntime) -> Result<(), NodeError> {
        rt.report_state(NodeRuntimeState::Idle, "waiting for configured field data");
        Ok(())
    }
    fn on_input(
        &mut self,
        ...
        let input = self
            ...
            .ok_or_else(|| {
                NodeError::Precondition(format!(
                    "i2cFieldEncoder received input on unknown port `{port}`"
                ))
            })?;
        let DataPacket::TypedField { datum, .. } = packet else {
            return Err(NodeError::Precondition(
                "i2cFieldEncoder inputs require data.field.v1".to_owned(),
            ));
        };
        if datum.name != input.datum_name {
            return Err(NodeError::Precondition(format!(
                "encoder port `{port}` expects datum `{}`, got `{}`",
                input.datum_name, datum.name
            )));
        }
        self.received.insert(port.to_owned(), datum);
        self.emit_task_if_ready(rt)
    }
    fn on_action(&mut self, action: NodeAction, _: &mut NodeRuntime) -> Result<(), NodeError> {
    ...
    fn on_config_update(
        ...
        rt.report_state(
            NodeRuntimeState::Idle,
            "I2C encoder configuration updated; send fields again",
        );
        Ok(())
    }
    fn on_stop(&mut self, rt: &mut NodeRuntime) -> Result<(), NodeError> {
...
impl NodeInstance for I2cTaskExecutorNode {
    ...
    fn on_start(&mut self, rt: &mut NodeRuntime) -> Result<(), NodeError> {
        rt.report_state(
            NodeRuntimeState::Idle,
            "waiting for task and SSH connection",
        );
        Ok(())
    }
    fn on_input(
        ...
        match (port, packet) {
            ...
            (CONNECTION_PORT, DataPacket::SshConnection(connection)) => {
                self.connection = Some(connection)
            }
            _ => {
                return Err(NodeError::Precondition(
                    "i2cTaskExecutor accepts only task PacketData and SSH connection".to_owned(),
                ));
            }
        }
        if self.task.is_some() && self.connection.is_some() {
            rt.report_state(NodeRuntimeState::Ready, "I2C task is ready to execute");
        }
        Ok(())
    }
    fn on_action(&mut self, action: NodeAction, rt: &mut NodeRuntime) -> Result<(), NodeError> {
        if !matches!(action, NodeAction::Custom { ref name, .. } if name == "execute") {
            return Err(NodeError::UnsupportedAction(action.name().to_owned()));
        }
        let task = self.task.as_ref().ok_or_else(|| {
            NodeError::Precondition("i2cTaskExecutor execute requires task".to_owned())
        })?;
        let connection = self.connection.as_ref().ok_or_else(|| {
            NodeError::Precondition("i2cTaskExecutor execute requires SSH connection".to_owned())
        })?;
...
impl I2cTaskPacket {
    fn with_digest(mut self) -> Self {
        self.final_image_digest = sha256_hex(&self.final_image);
        self
    }
    fn validate(&self) -> Result<(), String> {
        ...
        if self.map_id.trim().is_empty() || self.map_digest.trim().is_empty() {
            return Err("map identity must not be blank".to_owned());
        }
        if !matches!(self.target.address_width_bytes, 1 | 2)
            || !(0x03..=0x7f).contains(&self.target.address)
            || self.target.page_size_bytes == 0
        {
            return Err("target is invalid".to_owned());
        }
        if self.final_image.is_empty()
            || self.final_image.len() != usize::from(self.validation.image_bytes)
        {
            return Err("final image does not match validation image size".to_owned());
        }
        if self.final_image_digest != sha256_hex(&self.final_image) {
            return Err("final image digest does not match image bytes".to_owned());
        }
        if self.read_ranges.is_empty()
            || self.read_ranges.iter().any(|range| {
                range.byte_len == 0
                    || usize::from(range.offset).saturating_add(usize::from(range.byte_len))
                        > self.final_image.len()
            })
        {
            return Err("read ranges are invalid".to_owned());
        }
        if self.operation == TaskOperation::GuardedWrite && self.pages.is_empty() {
            return Err("guarded write task must contain pages".to_owned());
        }
        for page in &self.pages {
            if page.bytes.is_empty()
                || usize::from(page.offset).saturating_add(page.bytes.len())
                    > self.final_image.len()
                || page.settle_ms != self.target.write_cycle_ms
            {
                return Err("page layout is invalid".to_owned());
            }
        }
        Ok(())
    }
    fn read_request(&self) -> I2cReadRequest {
        I2cReadRequest::new(
            self.map_id.clone(),
            self.map_digest.clone(),
            self.target.clone(),
            self.read_ranges.clone(),
            self.validation.clone(),
        )
    }
    fn write_request(&self) -> I2cWriteRequest {
        I2cWriteRequest::new(
            self.map_id.clone(),
            self.map_digest.clone(),
            self.target.clone(),
...
fn parse_task(value: &serde_json::Value) -> Result<I2cTaskPacket, NodeError> {
    let task: I2cTaskPacket = serde_json::from_value(value.clone())
        .map_err(|error| NodeError::Precondition(format!("invalid I2C task JSON: {error}")))?;
    task.validate()
        .map_err(|error| NodeError::Precondition(format!("invalid I2C task JSON: {error}")))?;
    Ok(task)
}
...
fn compile_encoder_map(spec: &NodeSpec) -> Result<I2cMapDefinition, NodeError> {
    ...
    let mut map = match mode.as_str() {
        "builtin" => builtin_i2c_map(
            &config_text(spec, "mapId").unwrap_or_else(|| "yg-stereo-p24c64g-v1".to_owned()),
        )
        .ok_or_else(|| NodeError::Config("unsupported I2C map".to_owned()))?,
        "custom" => {
            I2cMapDefinition::from_yaml(&config_yaml(spec, "mapYaml").ok_or_else(|| {
                NodeError::Config("i2cFieldEncoder custom map requires mapYaml".to_owned())
            })?)
            .map_err(|error| NodeError::Config(format!("I2C map compilation failed: {error}")))?
        }
        _ => {
            return Err(NodeError::Config(
                "i2cFieldEncoder mapMode must be builtin or custom".to_owned(),
            ));
        }
    };
    map.target.bus = config_u32(spec, "bus")?.unwrap_or(0);
    let address = config_u16(spec, "address")?.ok_or_else(|| {
        NodeError::Config("i2cFieldEncoder config `address` is required".to_owned())
    })?;
    map.target.transport.i2c_address =
        u8::try_from(address).map_err(|_| NodeError::Config("address must fit u8".to_owned()))?;
    if let Some(width) = config_u16(spec, "addressWidthBytes")? {
        map.target.transport.address_width_bits = u8::try_from(
            width
                .checked_mul(8)
                .ok_or_else(|| NodeError::Config("addressWidthBytes overflow".to_owned()))?,
        )
        .map_err(|_| NodeError::Config("addressWidthBytes must be 1 or 2".to_owned()))?;
    }
    if let Some(size) = config_u16(spec, "pageSizeBytes")? {
        map.target.transport.page_size_bytes = size;
    }
    if let Some(cycle) = config_u16(spec, "writeCycleMs")? {
        map.target.transport.write_cycle_ms = cycle;
    }
    ...
    map.validate()
        .map_err(|error| NodeError::Config(format!("I2C encoder map is invalid: {error}")))?;
    Ok(map)
}

fn parse_encoder_inputs(
    spec: &NodeSpec,
    map: &I2cMapDefinition,
) -> Result<Vec<EncoderInput>, NodeError> {
    let configured = spec
        .config
        .get("inputs")
    ...
    let rows = if let Some(rows) = configured {
        ...
        map.inputs
            .iter()
            .map(|slot| EncoderInput {
                port_id: slot.name.clone(),
                datum_name: slot.name.clone(),
    ...
    if rows.is_empty() {
        return Err(NodeError::Config(
            "i2cFieldEncoder requires at least one input".to_owned(),
        ));
    }
    let mut ids = HashSet::new();
    for row in &rows {
        if !ids.insert(&row.port_id) || !map.inputs.iter().any(|slot| slot.name == row.datum_name) {
            return Err(NodeError::Config(format!(
                "encoder input `{}` is duplicate or not declared by map",
                row.port_id
            )));
        }
        let port = spec
            .inputs
            .iter()
            .find(|port| port.id == row.port_id)
            .ok_or_else(|| {
                NodeError::Config(format!("encoder input `{}` has no graph port", row.port_id))
            })?;
        if port.kind != FIELD_PORT_KIND {
            return Err(NodeError::Config(format!(
                "encoder input `{}` must use `{FIELD_PORT_KIND}`",
                row.port_id
            )));
        }
    }
    if spec.inputs.len() != rows.len() {
        return Err(NodeError::Config(
            "encoder graph inputs must exactly match configured inputs".to_owned(),
        ));
    }
    Ok(rows)
}
...
fn parse_operation(config: &serde_json::Value) -> Result<TaskOperation, NodeError> {
    match config
        ...
        .unwrap_or("guarded_write")
    {
        "read" => Ok(TaskOperation::Read),
        "guarded_write" => Ok(TaskOperation::GuardedWrite),
        value => Err(NodeError::Config(format!(
            "i2cFieldEncoder operation must be read or guarded_write, got `{value}`"
        ))),
    }
}
...
fn validate_encoder_ports(spec: &NodeSpec) -> Result<(), NodeError> {
    let task = spec
        .outputs
        .iter()
        .find(|port| port.id == TASK_PORT)
        .ok_or_else(|| NodeError::Config("i2cFieldEncoder requires task output".to_owned()))?;
    if task.kind != PACKET_PORT_KIND || spec.outputs.len() != 1 {
        return Err(NodeError::Config(
            "i2cFieldEncoder has exactly one data.packet.v1 task output".to_owned(),
        ));
    }
    Ok(())
}
...
fn validate_executor_ports(spec: &NodeSpec) -> Result<(), NodeError> {
        ...
        spec.inputs
            ...
            .ok_or_else(|| {
                NodeError::Config(format!(
                    "i2cTaskExecutor requires `{id}` input of kind `{kind}`"
                ))
            })
    };
    ...
    if spec.inputs.len() != 2 {
        return Err(NodeError::Config(
            "i2cTaskExecutor accepts exactly task and connection".to_owned(),
        ));
    }
    Ok(())
}
...
fn config_u16(spec: &NodeSpec, key: &str) -> Result<Option<u16>, NodeError> {
    match spec.config.get(key) {
        ...
        .map(Some)
        .map_err(|_| NodeError::Config(format!("config `{key}` must be u16"))),
    }
}
fn config_u32(spec: &NodeSpec, key: &str) -> Result<Option<u32>, NodeError> {
    match spec.config.get(key) {
        None => Ok(None),
        Some(value) => u32::try_from(
            value
                .as_u64()
                .ok_or_else(|| NodeError::Config(format!("config `{key}` must be u32")))?,
        )
        .map(Some)
        .map_err(|_| NodeError::Config(format!("config `{key}` must be u32"))),
    }
}
...
mod tests {
    ...
    fn task() -> I2cTaskPacket {
        ...
        I2cTaskPacket {
            schema: TASK_SCHEMA.to_owned(),
            operation: TaskOperation::GuardedWrite,
            map_id: map.id.clone(),
            map_digest: "sha256:map".to_owned(),
            target: target.clone(),
            read_ranges: vec![I2cReadRange {
                offset: 0,
                byte_len: map.image_bytes,
            }],
