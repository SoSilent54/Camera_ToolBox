...
|---|---:|---:|---:|---:|---:|---:|---:|---:|---|
| 正视平铺满 | 9 | 2.98e-05 | -24.767/-24.767 | -0.091/-0.111 | 5.83e+15 | 439.087 | 2.354 | 15950.893 | 焦距仍未充分约束 |
| 多姿态 | 21 | 2.92e-05 | -2.09e-06/8.03e-07 | 0.002/0.003 | 6.20e+15 | 6.90e-04 | 0.126 | 15305.614 | 畸变仍未充分约束 |
| 补深度 | 33 | 2.94e-05 | 4.84e-06/6.83e-06 | 5.24e-04/0.002 | -- | -- | -- | -- | 信息矩阵未满秩 |
| 补边角 | 81 | 2.96e-05 | 2.29e-06/2.27e-06 | 4.84e-05/2.77e-04 | 1.40e+13 | 1.25e-04 | 0.019 | 262.193 | 畸变仍未充分约束 |
...
- 焦距 σ 从 `439%` 降到 `0.00069%`。
- 主点 σ 从 `2.354px` 降到 `0.126px`。
...
补深度阶段加入近、中、远多个距离段。这里要区分“距离本身进不进公式”和“距离操作有没有工程作用”。内参方程不直接依赖某个绝对 $t_z$ 数值，但改变距离会改变棋盘成像尺度、透视强度和可达边缘位置。

阶段表现：第 32 张时焦距 σ 约 `0.00039%`，主点 σ 约 `0.080px`，比多姿态阶段继续下降。
...
边角阶段把棋盘推向图像大半径区域和四角。H2 指标出现最明显变化：

- 第 34 张：`D5 edge σ ≈ 3897px`
- 第 81 张：`D5 edge σ ≈ 262px`
...
- 但完整参数空间的 $H$ 仍病态，`cond(H)` 在 `1e13` 量级，`D5 edge σ` 仍远高于 `2px`。
...
- 标准递进和强边角数据证明“边缘/四角方向有效，但当前 D12 模型仍阻塞 H2 goal”。
...
- 深度变化主要帮助焦距尺度稳定。
- 边缘/四角是畸变解耦的关键。
- 当前 D12 全开会让 H2 长期病态，后续应重点验证 D5-first / staged-D12 策略。
